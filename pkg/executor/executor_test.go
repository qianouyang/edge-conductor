...
func TestNodeListUpdate(t *testing.T) {
	cases := []struct {
		name              string
		expectError       bool
		ekconfigparameter *pluginapi.EkconfigParameters
		funcBeforeTest    func() []*mpatch.Patch
	}{
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_ = executor.NodeListUpdate(tc.ekconfigparameter)
...
func patchNodeListUpdate(t *testing.T, fail bool) *mpatch.Patch {
	patch, patchErr := mpatch.PatchInstanceMethodByName(reflect.TypeOf(New()), "NodeListUpdate", func(e *Executor, params *pluginapi.EkconfigParameters) error {
		if fail {
			return errExEmpty
		} else {
			return nil
