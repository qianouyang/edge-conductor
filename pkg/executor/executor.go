...
var (
	errStringOverrideWithNode = fmt.Errorf("StringOverrideWithNode")
	errNodeLogin              = fmt.Errorf("Node login username missing")
	errNodeLoginPassword      = fmt.Errorf("Node login password and ssh key missing")
	errIgnoreOverride         = fmt.Errorf("Ignore override error")
	errIgnoreFormat           = fmt.Errorf("Ignore format error")
	errUnknownCmdType         = fmt.Errorf("Unknown command type")
)
...
func (e *Executor) StringOverrideWithNode(s string, ni *nodeInfo) (string, error) {
	...
	for _, n := range e.tempParams.Ekconfig.Parameters.Nodes {
		if n.IP == ni.ip {
			if n.User == "" {
	...
	if err != nil {
		...
		return "", errStringOverrideWithNode
	}
	log.Debugf("StringOverrideWithNode node: %v, s: %v, new: %v", ni.ip, s, c)
	return c, nil
}

func (e *Executor) NodeListUpdate(params *pluginapi.EkconfigParameters) error {
...
func (e *Executor) SetECParams(epparams *pluginapi.EpParams) error {
	...
	return e.NodeListUpdate(epparams.Ekconfig.Parameters)
}
