...
var (
	errNoRegistry         = errors.New("Registry server or port not found!")
	errNoAuth             = errors.New("no auth in docker client configuration file.")
	errInvalidPath        = errors.New("invalid config path, check the DOCKER_CONFIG or HOME env.")
	errDockerClientConfig = errors.New("Failed to open docker client config file")
	errParseCliCfg        = errors.New("Failed to parse cli config")
	errDecode             = fmt.Errorf("Failed to decode auth")
	errInvalidString      = fmt.Errorf("invalid auth string")
	errDockerCltCfg       = fmt.Errorf("docker client configuration file not found")
)
...
func getAuthFromCliConfig(registryIndex *registrytypes.IndexInfo) (*types.AuthConfig, error) {
	...
	if cliAuthConfigs != nil {
		...
		if authConfig.Auth == "" {
			return nil, errdefs.Unauthorized(errNoAuth)
		}
...
func getCliAuthConfig() (*map[string]types.AuthConfig, error) {
	...
	if !eputils.FileExists(configFilePath) {
		...
		return nil, errdefs.Unauthorized(errDockerCltCfg)
	}
	...
	if configCli, err := ioutil.ReadFile(configFilePath); err != nil {
		...
		return nil, errDockerClientConfig
	} else {
		...
		if err != nil {
			...
			return nil, errdefs.DataLoss(errParseCliCfg)
		} else {
			return &authConfig, nil
		}
	}
...
func parseAuth(authConfig *types.AuthConfig) error {
	...
	if e != nil {
		...
		return errdefs.DataLoss(errDecode)
	}
	...
	if len(parts) == 2 {
		...
		return errdefs.DataLoss(errInvalidString)
	}
}
