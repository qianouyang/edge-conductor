...
var (
	errWorkflow        = fmt.Errorf("No workflow found.")
	errSchemaInitData  = fmt.Errorf("Cannot find ep-params in init data")
	errSchemaOutData   = fmt.Errorf("Cannot find schema in output")
	errPreviousSchema  = fmt.Errorf("Cannot find schema in previous step's output or init data")
	errConvContainers  = fmt.Errorf("Convert containers data error")
	errMarshalInitData = fmt.Errorf("initData marshal error")
	errLoadData        = fmt.Errorf("Load ep-params data from init data error")
	errPluginData      = fmt.Errorf("Load plugin data from init data error")
	errMarshalPdata    = fmt.Errorf("pdata marshal error")
	errPluginReturn    = fmt.Errorf("Plugin return pdata error")
	errCmdNotSupported = fmt.Errorf("This command is not supported for current config.")
)
...
func (s *server) setInitData() error {
	...
	if data, has := s.plugin_dataattrs[name]; has {
		...
		return errSchemaInitData
	}
	json, err := initData.MarshalBinary()
	if err != nil {
		return errMarshalInitData
	}
	s.data.Data = json
	return nil
}
