...
var (
	epParam        = []byte(`{"ekconfig": {"Parameters": {"customconfig": {"registry": {"user": "a", "password": "b"}}, "global_settings": {"registry_port": "9000", "provider_ip": "test"}}}, "runtimedir": "aa", "workspace": "aa"}`)
	controllerNode = []byte(`{"ip":"aa", "ssh_port":"aa"}`)
	serviceConfig  = []byte(`{"components": [{"executor": { "deploy": "test" },"name": "castlelake", "type": "dce", "url": "file://release", "override": "http://override", "resources": [{"name": "helm", "value": "http://helm"}, {"name": "other", "value": "http://other"}, {"name": "etcd", "value": "http://etcd"} ]}]}`)

	// incorrectEpParam       = []byte(`{"ekconfig": {"Parameters": {"global_settings": {"registry_port": "9000", "provider_ip": "test"}}}, "runtimedir": "aa", "workspace": "aa"}`)
