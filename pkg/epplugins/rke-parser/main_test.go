...
func TestPluginMain(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expectedOutput: nil,
			expectError:    true,
			expectErrorMsg: "Manifest not found.",
		},
		{
			...
			expectedOutput: nil,
			expectError:    true,
			expectErrorMsg: "Failed to load json file!",
		},
		{
			...
			expectedOutput: nil,
			expectError:    true,
			expectErrorMsg: "Could not config the viper!",
		},
		{
			name: "RKE_parse_success",
			input: map[string][]byte{
				"ep-params":        []byte(`{"ekconfig": {"Cluster": {"provider": "rke", "config": "../../../configs/cluster-provider/rke_cluster.yml"}}}`),
				"cluster-manifest": cluster_manifest,
			},
