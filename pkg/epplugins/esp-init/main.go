...
var (
	errDownload    = errors.New(eputils.ERRORCODE_DOWNLOAD_ERROR)
	errInvalidFile = errors.New("file is not valid")
	errOSSession   = fmt.Errorf("No OS session in top config")
	errESPManifest = fmt.Errorf("ESP manifest Lost!")
	errESPCodebase = fmt.Errorf("ESP codebase not found")
	errESPBuild    = fmt.Errorf("ESP build.sh script not found")
	errESPRun      = fmt.Errorf("ESP run.sh script not found")
	errOSProvider  = fmt.Errorf("Wrong OS Provider: ")
)
...
func PluginMain(in eputils.SchemaMapData, outp *eputils.SchemaMapData) error {
	...
	if input_ep_params.Ekconfig.OS == nil {
		log.Errorf("No OS session in top config.")
		return errOSSession
	}

	if input_ep_params.Ekconfig.OS.Provider != "esp" {
		log.Errorf("Wrong OS Provider: %s", input_ep_params.Ekconfig.OS.Provider)
		return errOSProvider
	}

	if input_os_provider_manifest.Esp == nil {
		log.Errorln("Failed to find manifest for ESP os deployer.")
		return errESPManifest
	}

	// Get ESP Config from customcfg.yml
	userconfig := input_ep_params.Ekconfig.OS.Config
