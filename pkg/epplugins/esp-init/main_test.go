	...
	cases := []struct {
	...
	}{
		{
			...
			input: map[string][]byte{
				"ep-params":            []byte(`{"workspace":"testdata","ekconfig":{"OS": {"provider": "esp", "config": "testdata/fake-config2.yml"}}}`),
		...
		},
		{
			name: "success case2",
			input: map[string][]byte{
				"ep-params":            []byte(`{"workspace":"testdata","ekconfig":{"OS": {"provider": "esp", "config": "testdata/fake-config1.yml"}}}`),
				"os-provider-manifest": []byte(`{"esp":{"rel_url":"file://` + faketarball + `","rel_sha256":"aeb3883c4e817f1266cbbb4f6efc44490dbdd0e35e097a665abe54869c04fb38","rel_version":"1.0"}}`),
			},
			expectError:    false,
			expectErrorMsg: "",
		},
		{
			...
			input: map[string][]byte{
				"ep-params":            []byte(`{"workspace":"testdata","ekconfig":{"OS": {"provider": "esp", "config": "testdata/fake.cfg"}}}`),
				"os-provider-manifest": []byte(`{"esp":{"rel_url":"file://` + faketarball + `","rel_sha256":"aeb3883c4e817f1266cbbb4f6efc44490dbdd0e35e097a665abe54869c04fb38","rel_version":"1.0"}}`),
			},
			expectError:    true,
		...
		},
		{
			...
			input: map[string][]byte{
				"ep-params":            []byte(`{"ekconfig": {"global_settings": {"provider_ip": "test"}}}`),
				"os-provider-manifest": []byte(`{"esp":{"rel_url":"","rel_sha256":"","rel_version":""}}`),
			},
			expectError:    true,
			expectErrorMsg: "No OS session in top config",
		},
		{
			name: "Error Case: os provider not ESP",
			input: map[string][]byte{
				"ep-params":            []byte(`{"ekconfig":{"OS": {"provider": "none", "config": ""}}}`),
				"os-provider-manifest": []byte(`{"esp":{"rel_url":"","rel_sha256":"","rel_version":""}}`),
			},
			expectError:    true,
			expectErrorMsg: "Wrong OS Provider: ",
		},
		{
			name: "Error Case: no ESP manifest",
			input: map[string][]byte{
				"ep-params":            []byte(`{"ekconfig":{"OS": {"provider": "esp", "config": ""}}}`),
				"os-provider-manifest": []byte(`{}`),
			},
			expectError:    true,
			expectErrorMsg: "ESP manifest Lost!",
		},
		{
			name: "Error Case: invalid file",
			input: map[string][]byte{
				"ep-params":            []byte(`{"workspace":"testdata","ekconfig":{"OS": {"provider": "esp", "config": ""}}}`),
				"os-provider-manifest": []byte(`{"esp":{"rel_url":"file://testdata/nodata","rel_sha256":"","rel_version":""}}`),
			},
			expectError:    true,
			expectErrorMsg: eputils.ERRORCODE_DOWNLOAD_ERROR,
		},
		{
			...
			expectError:    true,
			expectErrorMsg: eputils.ERRORCODE_SHA256_CHECK_FAIL,
		},
		{
			name: "Error Case: no config",
			input: map[string][]byte{
				"ep-params":            []byte(`{"workspace":"testdata","ekconfig":{"OS": {"provider": "esp", "config": "nodata"}}}`),
				"os-provider-manifest": []byte(`{"esp":{"rel_url":"file://` + faketarball + `","rel_sha256":"aeb3883c4e817f1266cbbb4f6efc44490dbdd0e35e097a665abe54869c04fb38","rel_version":"1.0"}}`),
			},
			expectError:    true,
