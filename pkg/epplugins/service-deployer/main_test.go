...
var (
	...
	errHelmStatus      = errors.New("Helm service status unknown")
	errHelmWrongStatus = errors.New("Helm service with wrong status")
)
...
func Test_getRevision(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			expectedErr: errHelmWrongStatus,
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
