...
func TestPluginMain(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			input: map[string][]byte{
				"ep-params": []byte(`{"ekconfig":{"Cluster":{"Provider":"kind"},"Components":{"selector":[{"name": "testhelmrepo"}],"manifests":["test-chart-name.yml"]}}}`),
			},
			expectedOutput: nil,
			expectError:    true,
			expectErrorMsg: "Helm repo or chart name is empty",
			funcBeforeTest: nil,
		},
		{
			name: "err_empty_HelmRepo",
			input: map[string][]byte{
				"ep-params": []byte(`{"ekconfig":{"Cluster":{"Provider":"kind"},"Components":{"selector":[{"name": "testhelmrepo"}],"manifests":["test-helm-repo.yml"]}}}`),
			},
			expectedOutput: nil,
			expectError:    true,
			expectErrorMsg: "Helm repo or chart name is empty",
