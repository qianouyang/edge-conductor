...
import (
	...
	fakeserviceutils "ep/pkg/eputils/test/fakeserviceutils"
...
func TestPluginMain(t *testing.T) {
	...
	func_get_capi_template_fail := func(ctrl *gomock.Controller) []*mpatch.Patch {
		...
		p2, err := mpatch.PatchMethod(capiutils.GetCapiSetting, func(*pluginapi.EpParams, *pluginapi.Clustermanifest, *pluginapi.CapiClusterConfig, *pluginapi.CapiSetting) {
		})
		if err != nil {
			t.Errorf("patch error: %v", err)
			return nil
