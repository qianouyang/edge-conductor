...
//nolint:deadcode,unused
func generate_input_rkeconfig(data []byte, in eputils.SchemaMapData) bool {
	...
	in[__name("rkeconfig")] = inputStruct
	return true
}
...
func generateInput(data map[string][]byte) eputils.SchemaMapData {
	...
	if result := generate_input_rkeconfig(data["rkeconfig"], n); !result {
		return nil
	}
