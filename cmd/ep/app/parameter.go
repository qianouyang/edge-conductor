...
var (
	errEpParam    = errors.New("epParams not found.")
	errEkconfig   = errors.New("epParams.Ekconfig not found.")
	errConfigPath = errors.New("EK Config path not provided.")
	errRegistryPw = errors.New("No registry password for admin found.")
)
...
func setupCustomConfig(ekcfgPath string, epp *epapiplugins.EpParams) error {
	var ekcfg epapiplugins.Ekconfig
	if epp == nil {
		return errEpParam
	} else if epp.Ekconfig == nil {
		return errEkconfig
	}
	if ekcfgPath == "" {
		if epp.Ekconfigpath == "" {
			...
			ekcfgPath = epp.Ekconfigpath
		}
	} else {
		epp.Ekconfigpath = ekcfgPath
	}
	if _, err := os.Stat(ekcfgPath); os.IsNotExist(err) {
		log.Error("Failed to open", ekcfgPath, err)
		return err
	}
	if err := load_ek_config(&ekcfg, ekcfgPath); err != nil {
		log.Error(err)
		return err
	}

	// Load node list from user config file.
	epp.Ekconfig.Parameters.Nodes = ekcfg.Parameters.Nodes

	// Load Customconfig from user config file.
	epp.Ekconfig.Parameters.Customconfig = ekcfg.Parameters.Customconfig
	ctmcfg := epp.Ekconfig.Parameters.Customconfig
	...
	if needUserPassword {
		...
		return errRegistryPw
	}
	return nil
}

func teardownCustomConfig(epp *epapiplugins.EpParams) {
	epp.Ekconfig.Parameters.Customconfig = &epapiplugins.Customconfig{}
	epp.Ekconfig.Parameters.Nodes = []*epapiplugins.Node{}
}

func InitEpParams(topcfg epapiplugins.Ekconfig) *epapiplugins.EpParams {
	...
	for _, ext := range topcfg.Parameters.Extensions {
		...
		var extPath string
		log.Info("extension in ek is ", ext)
		foundExt := false
