...
func patchInitTopConfig(t *testing.T, err error) *mpatch.Patch {
	patch, patchErr := mpatch.PatchMethod(init_ek_config, func() error {
		return err
	})
	if patchErr != nil {
...
func patch_load_ek_bcfg(t *testing.T, err error) *mpatch.Patch {
	patch, patchErr := mpatch.PatchMethod(load_ek_bcfg, func(file string) (*EKBaseConfig, error) {
		return &EKBaseConfig{}, err
	})
	if patchErr != nil {
		t.Errorf("patch error: %v", patchErr)
		return nil
...
func patchEkconfigValidate(t *testing.T, err error) *mpatch.Patch {
	patch, patchErr := mpatch.PatchInstanceMethodByName(reflect.TypeOf(&epapiplugins.Ekconfig{}), "Validate", func(_ *epapiplugins.Ekconfig, formats strfmt.Registry) error {
		return err
	})
	if patchErr != nil {
		t.Errorf("patch error: %v", patchErr)
...
func getTemplateCfg() epapiplugins.Ekconfig {
	return epapiplugins.Ekconfig{
		...
		Components: &epapiplugins.EkconfigComponents{
			...
			Selector: []*plugins.EkconfigComponentsSelectorItems0{
				{Name: "portainer"},
	...
	cases := []struct {
		...
		},
		{
			funcBeforeTest: func() []*mpatch.Patch {
				...
				patch2 := patchEkconfigValidate(t, testError)
				ekcfg.Cluster = &plugins.EkconfigCluster{Config: fileName}
				ekcfg.Components = &plugins.EkconfigComponents{}
...
func TestInitTopConfig(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			funcBeforeTest: func() []*mpatch.Patch {
				ekcfg = epapiplugins.Ekconfig{}
				patch := patch_load_ek_bcfg(t, testError)
				return []*mpatch.Patch{patch}
			},
			isFunctionCorrectly: func(err error) {
	...
	for n, testCase := range cases {
		...
		func() {
			...
			testCase.isFunctionCorrectly(init_ek_config())
		}()
		t.Logf("%s case %d End", getFuncName(), n)
	}
