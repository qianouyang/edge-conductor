...
func TestEpWfStart(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			wantError:           errTopConf,
			isFunctionCorrectly: isFunctionCorrectlyFunc,
			funcBeforeTest: func() []*mpatch.Patch {
				patch, patchErr := mpatch.PatchMethod(GetRuntimeTopConfig, func(epParams *epapiplugins.EpParams) *epapiplugins.Ekconfig {
					return nil
				})
...
func TestEpWfPreInit(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			epPms:     nil,
			funcBeforeTest: func() []*mpatch.Patch {
				patch := patchFileNameofRuntime(t, "", testError)
				return []*mpatch.Patch{patch}
			},
			isFunctionCorrectly: isFunctionCorrectlyFunc,
		},
		{
			...
			funcBeforeTest: func() []*mpatch.Patch {
				...
				patchStat := patchStat(t, nil, os.ErrNotExist)
				patch_load_ek_bcfg := patch_load_ek_bcfg(t, nil)
				return []*mpatch.Patch{patchFileNameofRuntime, patchStat, patch_load_ek_bcfg}
			},
			isFunctionCorrectly: isFunctionCorrectlyFunc,
		},
		{
...
func TestEpwfLoadServices(t *testing.T) {
	func_load_services_fail := func(ctrl *gomock.Controller) []*mpatch.Patch {
		pathchLoadEkServices, err := mpatch.PatchMethod(load_ek_services, func(ekconfig *epapiplugins.Ekconfig, ekfile string) error {
			return errTest
		})
		if err != nil {
		...
		// TODO: Add test cases.
		{
			...
			args: args{
				&epapiplugins.EpParams{
					...
					Ekconfigpath: "/home/user/testPath",
				},
			},
