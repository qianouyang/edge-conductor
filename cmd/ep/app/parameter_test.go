...
func Test_setupcustomconfig(t *testing.T) {
	cases := []struct {
		...
		{
			name:         "ekcfgpath is not valid",
			expectError:  errEmpty,
			in_ekcfgPath: "",
			in_epp:       getepparams(t, "epparams_withoutctmcfg.json"),
		},
		{
			name:         "ekcfgpath not provided",
			expectError:  errConfigPath,
			in_ekcfgPath: "",
			in_epp:       getepparams(t, "epparams_withctmcfg.json"),
		},
		{
			name:         "need user password",
			expectError:  errRegistryPw,
			in_ekcfgPath: getrealpath("ctmcfg_withloginfail.yml"),
		...
		},
		{
			...
			expectError:  nil,
			in_ekcfgPath: getrealpath("ctmcfg_withloginok.yml"),
...

		{
			...
			expectError:  nil,
			in_ekcfgPath: getrealpath("ctmcfg_withloginok_components.yml"),
			in_epp:       getepparams(t, "epparams_withadminpass.json"),
			beforetest: func() {
				patchOsStat(t, nil, nil)
				patchisnotexist(t, false)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, tc := range cases {
				...
				err := setupCustomConfig(tc.in_ekcfgPath, tc.in_epp)
...
//nolint:unparam
func getekconfig(t *testing.T, configfile string) epapiplugins.Ekconfig {
	_, pwdpath, _, _ := runtime.Caller(0)
	testdatapath := filepath.Join(filepath.Dir(pwdpath), "testdata")
	...
	err = ekconfig.UnmarshalBinary(b)
	if err != nil {
		t.Errorf("Can't UnmarshalBinary %v", configfile)
	}
	return ekconfig
...
func Test_InitEpParams(t *testing.T) {
	...
	cases := []struct {
		...
		in_eptopconfg   epapiplugins.Ekconfig
		beforetest      func()
		teardown        func()
	}{
		{
			...
			in_eptopconfg:   getekconfig(t, "ekconfig_sample.json"),
			beforetest: func() {
